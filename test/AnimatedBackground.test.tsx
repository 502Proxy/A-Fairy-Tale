...
const mockCtx = {
  ...
  set strokeStyle(value: string | CanvasGradient | CanvasPattern) {
    this._strokeStyle = String(value);
  },
  ...
  it('should get the canvas context and set dimensions on mount', async () => {
    ...
    await waitFor(() => {
      ...
      expect(canvasElement).toHaveAttribute(
        'width',
        `${initialWidth * initialDPR}`
      );
      expect(canvasElement).toHaveAttribute(
        'height',
        `${initialHeight * initialDPR}`
      );
    });
  });
    ...
    try {
      await waitFor(() => {
        ...
        if (!addedListener || animationFrameId === undefined) {
          throw new Error(
            'Listener or Frame ID not captured yet within waitFor'
          );
        }
      });
    } catch (error) {
